function initialize() {
	...
	//Set dom element for render
	var domElement = document.getElementById("object");

	//Set main reference
	const databaseReference = firebase.database().ref();

	//List all main tables in database for console
	databaseReference.on('value', function(snapshot) {
		snapshot.forEach(firstSnapshot => {
			var childKey = firstSnapshot.key;
			var childData = firstSnapshot.val();
	...
	locationsRef.on('child_added', function(snapshot) {
	  ...
	  marker.addListener('click', (function(data) {
		return function(e) {
		  infowindow.setContent('<div class="map-custom-window">' + '<span class="window__label"> İsim: </span>' + data.name  + '<span class="window__label">Koordinat: </span>' + this.getPosition().toUrlValue(6) +  '<span class="window__label">Mesaj: </span>' + data.message + `<img class="map-custom-window__img" src="${data.image}">` + '</div>');
